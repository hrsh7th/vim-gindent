...
---@enum gindent.kit.LSP.CodeActionTag
LSP.CodeActionTag = {
  LLMGenerated = 1,
}

---@enum gindent.kit.LSP.TraceValue
LSP.TraceValue = {
  Off = 'off',
  Messages = 'messages',
  Verbose = 'verbose',
...
---@enum gindent.kit.LSP.LanguageKind
LSP.LanguageKind = {
  ABAP = 'abap',
  WindowsBat = 'bat',
  BibTeX = 'bibtex',
...
---@enum gindent.kit.LSP.InlineCompletionTriggerKind
...
---@field public event gindent.kit.LSP.WorkspaceFoldersChangeEvent The actual workspace folder change event.
...
---@field public textEdit? gindent.kit.LSP.TextEdit An {@link TextEdit edit} which is applied to a document when selecting<br>this presentation for the color.  When `falsy` the {@link ColorPresentation.label label}<br>is used.
---@field public additionalTextEdits? gindent.kit.LSP.TextEdit[] An optional array of additional {@link TextEdit text edits} that are applied when<br>selecting this color presentation. Edits must not overlap with the main {@link ColorPresentation.textEdit edit} nor with themselves.
...
---@field public kind? gindent.kit.LSP.FoldingRangeKind Describes the kind of the folding range such as 'comment' or 'region'. The kind<br>is used to categorize folding ranges and used by commands like 'Fold all comments'.<br>See {@link FoldingRangeKind} for an enumeration of standardized kinds.
...
---@field public range gindent.kit.LSP.Range The {@link Range range} of this selection range.
...
---@field public kind gindent.kit.LSP.SymbolKind The kind of this item.
---@field public tags? gindent.kit.LSP.SymbolTag[] Tags for this item.
...
---@field public range gindent.kit.LSP.Range The range enclosing this symbol not including leading/trailing whitespace but everything else, e.g. comments and code.
---@field public selectionRange gindent.kit.LSP.Range The range that should be selected and revealed when this symbol is being picked, e.g. the name of a function.<br>Must be contained by the {@link CallHierarchyItem.range `range`}.
...
---@field public fromRanges gindent.kit.LSP.Range[] The ranges at which the calls appear. This is relative to the caller<br>denoted by {@link CallHierarchyIncomingCall.from `this.from`}.
...
---@field public edits gindent.kit.LSP.SemanticTokensEdit[] The semantic token edits to transform a previous result into a new result.
...
---@field public selection? gindent.kit.LSP.Range An optional selection range if the document is a text<br>document. Clients might ignore the property if an<br>external program is started or the file is not a text<br>file.
...
---@field public detail? string More detail for this item, e.g. the signature of a function.
---@field public uri string The resource identifier of this item.
---@field public range gindent.kit.LSP.Range The range enclosing this symbol not including leading/trailing whitespace<br>but everything else, e.g. comments and code.
---@field public selectionRange gindent.kit.LSP.Range The range that should be selected and revealed when this symbol is being<br>picked, e.g. the name of a function. Must be contained by the<br>{@link TypeHierarchyItem.range `range`}.
...
---@field public position gindent.kit.LSP.Position The position of this hint.<br><br>If multiple hints have the same position, they will be shown in the order<br>they appear in the response.
---@field public label (string | gindent.kit.LSP.InlayHintLabelPart[]) The label of this hint. A human readable string or an array of<br>InlayHintLabelPart label parts.<br><br>*Note* that neither the string nor the label part can be empty.
---@field public kind? gindent.kit.LSP.InlayHintKind The kind of this hint. Can be omitted in which case the client<br>should fall back to a reasonable default.
---@field public textEdits? gindent.kit.LSP.TextEdit[] Optional text edits that are performed when accepting this inlay hint.<br><br>*Note* that edits are expected to change the document so that the inlay<br>hint (or its nearest variant) is now part of the document and the inlay<br>hint itself is now obsolete.
---@field public tooltip? (string | gindent.kit.LSP.MarkupContent) The tooltip text when you hover over this item.
...
---@field public notebookDocument gindent.kit.LSP.NotebookDocument The notebook document that got opened.
---@field public cellTextDocuments gindent.kit.LSP.TextDocumentItem[] The text documents that represent the content<br>of a notebook cell.
...
---@field public notebookDocument gindent.kit.LSP.VersionedNotebookDocumentIdentifier The notebook document that did change. The version number points<br>to the version after all provided changes have been applied. If<br>only the text document content of a cell changes the notebook version<br>doesn't necessarily have to change.
---@field public change gindent.kit.LSP.NotebookDocumentChangeEvent The actual changes to the notebook document.<br><br>The changes describe single state changes to the notebook document.<br>So if there are two changes c1 (at array index 0) and c2 (at array<br>index 1) for a notebook in state S then c1 moves the notebook from<br>S to S' and c2 from S' to S''. So c1 is computed on the state S and<br>c2 is computed on the state S'.<br><br>To mirror the content of a notebook using change events use the following approach:<br>- start with the same initial content<br>- apply the 'notebookDocument/didChange' notifications in the order you receive them.<br>- apply the `NotebookChangeEvent`s in a single notification in the order<br>  you receive them.

---@class gindent.kit.LSP.DidSaveNotebookDocumentParams
---@field public notebookDocument gindent.kit.LSP.NotebookDocumentIdentifier The notebook document that got saved.

---@class gindent.kit.LSP.DidCloseNotebookDocumentParams
---@field public notebookDocument gindent.kit.LSP.NotebookDocumentIdentifier The notebook document that got closed.
---@field public cellTextDocuments gindent.kit.LSP.TextDocumentIdentifier[] The text documents that represent the content<br>of a notebook cell that got closed.

---@class gindent.kit.LSP.InlineCompletionParams : gindent.kit.LSP.TextDocumentPositionParams, gindent.kit.LSP.WorkDoneProgressParams
---@field public context gindent.kit.LSP.InlineCompletionContext Additional information about the context in which inline completions were<br>requested.

---@class gindent.kit.LSP.InlineCompletionList
---@field public items gindent.kit.LSP.InlineCompletionItem[] The inline completion items

---@class gindent.kit.LSP.InlineCompletionItem
---@field public insertText (string | gindent.kit.LSP.StringValue) The text to replace the range with. Must be set.
...
---@field public range? gindent.kit.LSP.Range The range to replace. Must begin and end on the same line.
---@field public command? gindent.kit.LSP.Command An optional {@link Command} that is executed *after* inserting this completion.

---@class gindent.kit.LSP.InlineCompletionRegistrationOptions : gindent.kit.LSP.InlineCompletionOptions, gindent.kit.LSP.TextDocumentRegistrationOptions, gindent.kit.LSP.StaticRegistrationOptions
...
---@field public serverInfo? gindent.kit.LSP.ServerInfo Information about the server.<br><br>@since 3.15.0
...
---@field public settings gindent.kit.LSP.LSPAny The actual changed settings
...
---@field public type gindent.kit.LSP.MessageType The message type. See {@link MessageType}
---@field public message string The actual message.

---@class gindent.kit.LSP.ShowMessageRequestParams
---@field public type gindent.kit.LSP.MessageType The message type. See {@link MessageType}
---@field public message string The actual message.
---@field public actions? gindent.kit.LSP.MessageActionItem[] The message action items to present.
...
---@field public textDocument gindent.kit.LSP.TextDocumentItem The document that was opened.

---@class gindent.kit.LSP.DidChangeTextDocumentParams
---@field public textDocument gindent.kit.LSP.VersionedTextDocumentIdentifier The document that did change. The version number points<br>to the version after all provided content changes have<br>been applied.
---@field public contentChanges gindent.kit.LSP.TextDocumentContentChangeEvent[] The actual content changes. The content changes describe single state changes<br>to the document. So if there are two content changes c1 (at array index 0) and<br>c2 (at array index 1) for a document in state S then c1 moves the document from<br>S to S' and c2 from S' to S''. So c1 is computed on the state S and c2 is computed<br>on the state S'.<br><br>To mirror the content of a document using change events use the following approach:<br>- start with the same initial content<br>- apply the 'textDocument/didChange' notifications in the order you receive them.<br>- apply the `TextDocumentContentChangeEvent`s in a single notification in the order<br>  you receive them.

---@class gindent.kit.LSP.TextDocumentChangeRegistrationOptions : gindent.kit.LSP.TextDocumentRegistrationOptions
---@field public syncKind gindent.kit.LSP.TextDocumentSyncKind How documents are synced to the server.

---@class gindent.kit.LSP.DidCloseTextDocumentParams
---@field public textDocument gindent.kit.LSP.TextDocumentIdentifier The document that was closed.
...
---@field public reason gindent.kit.LSP.TextDocumentSaveReason The 'TextDocumentSaveReason'.

---@class gindent.kit.LSP.TextEdit
---@field public range gindent.kit.LSP.Range The range of the text document to be manipulated. To insert<br>text into a document create a range where start === end.
...
---@field public changes gindent.kit.LSP.FileEvent[] The actual file events.

---@class gindent.kit.LSP.DidChangeWatchedFilesRegistrationOptions
---@field public watchers gindent.kit.LSP.FileSystemWatcher[] The watchers to register.
...
---@field public context? gindent.kit.LSP.CompletionContext The completion context. This is only available if the client specifies<br>to send this using the client capability `textDocument.completion.contextSupport === true`
...
---@field public labelDetails? gindent.kit.LSP.CompletionItemLabelDetails Additional details for the label<br><br>@since 3.17.0
---@field public kind? gindent.kit.LSP.CompletionItemKind The kind of this completion item. Based of the kind<br>an icon is chosen by the editor.
---@field public tags? gindent.kit.LSP.CompletionItemTag[] Tags for this completion item.<br><br>@since 3.15.0
...
---@field public documentation? (string | gindent.kit.LSP.MarkupContent) A human-readable string that represents a doc-comment.
...
---@field public command? gindent.kit.LSP.Command An optional {@link Command command} that is executed *after* inserting this completion. *Note* that<br>additional modifications to the current document should be described with the<br>{@link CompletionItem.additionalTextEdits additionalTextEdits}-property.
---@field public data? gindent.kit.LSP.LSPAny A data entry field that is preserved on a completion item between a<br>{@link CompletionRequest} and a {@link CompletionResolveRequest}.
...
---@field public signatures gindent.kit.LSP.SignatureInformation[] One or more signatures.
...
---@field public kind? gindent.kit.LSP.DocumentHighlightKind The highlight kind, default is {@link DocumentHighlightKind.Text text}.
...
---@field public kind gindent.kit.LSP.SymbolKind The kind of this symbol.
---@field public tags? gindent.kit.LSP.SymbolTag[] Tags for this document symbol.<br><br>@since 3.16.0
---@field public deprecated? boolean Indicates if this symbol is deprecated.<br><br>@deprecated Use tags instead
---@field public range gindent.kit.LSP.Range The range enclosing this symbol not including leading/trailing whitespace but everything else<br>like comments. This information is typically used to determine if the clients cursor is<br>inside the symbol to reveal in the symbol in the UI.
...
---@field public arguments? gindent.kit.LSP.LSPAny[] Arguments that the command handler should be<br>invoked with.

---@class gindent.kit.LSP.CodeAction
---@field public title string A short, human-readable, title for this code action.
---@field public kind? gindent.kit.LSP.CodeActionKind The kind of the code action.<br><br>Used to filter code actions.
---@field public diagnostics? gindent.kit.LSP.Diagnostic[] The diagnostics that this code action resolves.
...
---@field public disabled? gindent.kit.LSP.CodeActionDisabled Marks that the code action cannot currently be applied.<br><br>Clients should follow the following guidelines regarding disabled code actions:<br><br>  - Disabled code actions are not shown in automatic [lightbulbs](https://code.visualstudio.com/docs/editor/editingevolved#_code-action)<br>    code action menus.<br><br>  - Disabled actions are shown as faded out in the code action menu when the user requests a more specific type<br>    of code action, such as refactorings.<br><br>  - If the user has a [keybinding](https://code.visualstudio.com/docs/editor/refactoring#_keybindings-for-code-actions)<br>    that auto applies a code action and only disabled code actions are returned, the client should show the user an<br>    error message with `reason` in the editor.<br><br>@since 3.16.0
---@field public edit? gindent.kit.LSP.WorkspaceEdit The workspace edit this code action performs.
---@field public command? gindent.kit.LSP.Command A command this code action executes. If a code action<br>provides an edit and a command, first the edit is<br>executed and then the command.
---@field public data? gindent.kit.LSP.LSPAny A data entry field that is preserved on a code action between<br>a `textDocument/codeAction` and a `codeAction/resolve` request.<br><br>@since 3.16.0
---@field public tags? gindent.kit.LSP.CodeActionTag[] Tags for this code action.<br><br>@since 3.18.0 - proposed
...
---@field public location (gindent.kit.LSP.Location | gindent.kit.LSP.LocationUriOnly) The location of the symbol. Whether a server is allowed to<br>return a location without a range depends on the client<br>capability `workspace.symbol.resolveSupport`.<br><br>See SymbolInformation#location for more details.
---@field public data? gindent.kit.LSP.LSPAny A data entry field that is preserved on a workspace symbol between a<br>workspace symbol request and a workspace symbol resolve request.
...
---@field public range gindent.kit.LSP.Range The range in which this code lens is valid. Should only span a single line.
---@field public command? gindent.kit.LSP.Command The command this code lens represents.
---@field public data? gindent.kit.LSP.LSPAny A data entry field that is preserved on a code lens item between<br>a {@link CodeLensRequest} and a {@link CodeLensResolveRequest}
...
---@field public range gindent.kit.LSP.Range The range this link applies to.
...
---@class gindent.kit.LSP.DocumentRangesFormattingParams : gindent.kit.LSP.WorkDoneProgressParams
---@field public textDocument gindent.kit.LSP.TextDocumentIdentifier The document to format.
---@field public ranges gindent.kit.LSP.Range[] The ranges to format
---@field public options gindent.kit.LSP.FormattingOptions The format options

---@class gindent.kit.LSP.DocumentOnTypeFormattingParams
---@field public textDocument gindent.kit.LSP.TextDocumentIdentifier The document to format.
---@field public position gindent.kit.LSP.Position The position around which the on type formatting should happen.<br>This is not necessarily the exact position where the character denoted<br>by the property `ch` got typed.
...
---@field public edit gindent.kit.LSP.WorkspaceEdit The edits to apply.
---@field public metadata? gindent.kit.LSP.WorkspaceEditMetadata Additional data about the edit.<br><br>@since 3.18.0<br>@proposed
...
---@field public value gindent.kit.LSP.TraceValue
...
---@field public token gindent.kit.LSP.ProgressToken The progress token provided by the client or server.
---@field public value gindent.kit.LSP.LSPAny The progress data.

---@class gindent.kit.LSP.TextDocumentPositionParams
---@field public textDocument gindent.kit.LSP.TextDocumentIdentifier The text document.
---@field public position gindent.kit.LSP.Position The position inside the text document.

---@class gindent.kit.LSP.WorkDoneProgressParams
---@field public workDoneToken? gindent.kit.LSP.ProgressToken An optional token that a server can use to report work done progress.
...
---@field public originSelectionRange? gindent.kit.LSP.Range Span of the origin of this link.<br><br>Used as the underlined span for mouse interaction. Defaults to the word range at<br>the definition position.
---@field public targetUri string The target resource identifier of this link.
---@field public targetRange gindent.kit.LSP.Range The full target range of this link. If the target for example is a symbol then target range is the<br>range enclosing this symbol not including leading/trailing whitespace but everything else<br>like comments. This information is typically used to highlight the range in the editor.
---@field public targetSelectionRange gindent.kit.LSP.Range The range that should be selected and revealed when this link is being followed, e.g the name of a function.<br>Must be contained by the `targetRange`. See also `DocumentSymbol#range`
...
---@field public removed gindent.kit.LSP.WorkspaceFolder[] The array of the removed workspace folders
...
---@field public full? (boolean | gindent.kit.LSP.SemanticTokensFullDelta) Server supports providing semantic tokens for a full document.
...
---@field public textDocument gindent.kit.LSP.OptionalVersionedTextDocumentIdentifier The text document to change.
---@field public edits (gindent.kit.LSP.TextEdit | gindent.kit.LSP.AnnotatedTextEdit | gindent.kit.LSP.SnippetTextEdit)[] The edits to be applied.<br><br>@since 3.16.0 - support for AnnotatedTextEdit. This is guarded using a<br>client capability.<br><br>@since 3.18.0 - support for SnippetTextEdit. This is guarded using a<br>client capability.
...
---@field public options? gindent.kit.LSP.CreateFileOptions Additional options
...
---@field public options? gindent.kit.LSP.RenameFileOptions Rename options.
...
---@field public options? gindent.kit.LSP.DeleteFileOptions Delete options.
...
---@field public pattern gindent.kit.LSP.FileOperationPattern The actual file operation pattern.
...
---@field public stoppedLocation gindent.kit.LSP.Range The document range where execution has stopped.<br>Typically the end position of the range denotes the line where the inline values are shown.

---@class gindent.kit.LSP.InlineValueText
---@field public range gindent.kit.LSP.Range The document range for which the inline value applies.
---@field public text string The text of the inline value.

---@class gindent.kit.LSP.InlineValueVariableLookup
---@field public range gindent.kit.LSP.Range The document range for which the inline value applies.<br>The range is used to extract the variable name from the underlying document.
...
---@field public tooltip? (string | gindent.kit.LSP.MarkupContent) The tooltip text when you hover over this label part. Depending on<br>the client capability `inlayHint.resolveSupport` clients might resolve<br>this property late using the resolve request.
---@field public location? gindent.kit.LSP.Location An optional source code location that represents this<br>label part.<br><br>The editor will use this location for the hover and for code navigation<br>features: This part will become a clickable link that resolves to the<br>definition of the symbol at the given location (not necessarily the<br>location itself), it shows the hover that shows at the given location,<br>and it shows a context menu with further code navigation commands.<br><br>Depending on the client capability `inlayHint.resolveSupport` clients<br>might resolve this property late using the resolve request.
---@field public command? gindent.kit.LSP.Command An optional command for this label part.<br><br>Depending on the client capability `inlayHint.resolveSupport` clients<br>might resolve this property late using the resolve request.
...
---@field public items gindent.kit.LSP.Diagnostic[] The actual items.
...
---@field public metadata? gindent.kit.LSP.LSPObject Additional metadata stored with the notebook<br>document.<br><br>Note: should always be an object literal (e.g. LSPObject)
---@field public cells gindent.kit.LSP.NotebookCell[] The cells of a notebook.

---@class gindent.kit.LSP.TextDocumentItem
---@field public uri string The text document's uri.
---@field public languageId gindent.kit.LSP.LanguageKind The text document's language identifier.
---@field public version integer The version number of this document (it will increase after each<br>change, including undo/redo).
---@field public text string The content of the opened text document.

---@class gindent.kit.LSP.NotebookDocumentSyncOptions
---@field public notebookSelector (gindent.kit.LSP.NotebookDocumentFilterWithNotebook | gindent.kit.LSP.NotebookDocumentFilterWithCells)[] The notebooks to be synced
...
---@field public metadata? gindent.kit.LSP.LSPObject The changed meta data if any.<br><br>Note: should always be an object literal (e.g. LSPObject)
---@field public cells? gindent.kit.LSP.NotebookDocumentCellChanges Changes to cells

---@class gindent.kit.LSP.NotebookDocumentIdentifier
---@field public uri string The notebook document's uri.

---@class gindent.kit.LSP.InlineCompletionContext
---@field public triggerKind gindent.kit.LSP.InlineCompletionTriggerKind Describes how the inline completion was triggered.
---@field public selectedCompletionInfo? gindent.kit.LSP.SelectedCompletionInfo Provides information about the currently selected item in the autocomplete widget if it is visible.

---@class gindent.kit.LSP.StringValue
---@field public kind "snippet" The kind of string value.
---@field public value string The snippet string.

---@class gindent.kit.LSP.InlineCompletionOptions : gindent.kit.LSP.WorkDoneProgressOptions
...
---@field public registerOptions? gindent.kit.LSP.LSPAny Options necessary for the registration.
...
---@field public clientInfo? gindent.kit.LSP.ClientInfo Information about the client<br><br>@since 3.15.0
...
---@field public capabilities gindent.kit.LSP.ClientCapabilities The capabilities provided by the client (editor or tool)
---@field public initializationOptions? gindent.kit.LSP.LSPAny User provided initialization options.
---@field public trace? gindent.kit.LSP.TraceValue The initial trace setting. If omitted trace is disabled ('off').

---@class gindent.kit.LSP.WorkspaceFoldersInitializeParams
---@field public workspaceFolders? (gindent.kit.LSP.WorkspaceFolder[] | nil) The workspace folders configured in the client when the server starts.<br><br>This property is only available if the client supports workspace folders.<br>It can be `null` if the client supports workspace folders but none are<br>configured.<br><br>@since 3.6.0

---@class gindent.kit.LSP.ServerCapabilities
---@field public positionEncoding? gindent.kit.LSP.PositionEncodingKind The position encoding the server picked from the encodings offered<br>by the client via the client capability `general.positionEncodings`.<br><br>If the client didn't provide any position encodings the only valid<br>value that a server can return is 'utf-16'.<br><br>If omitted it defaults to 'utf-16'.<br><br>@since 3.17.0
---@field public textDocumentSync? (gindent.kit.LSP.TextDocumentSyncOptions | gindent.kit.LSP.TextDocumentSyncKind) Defines how text documents are synced. Is either a detailed structure<br>defining each notification or for backwards compatibility the<br>TextDocumentSyncKind number.
---@field public notebookDocumentSync? (gindent.kit.LSP.NotebookDocumentSyncOptions | gindent.kit.LSP.NotebookDocumentSyncRegistrationOptions) Defines how notebook documents are synced.<br><br>@since 3.17.0
---@field public completionProvider? gindent.kit.LSP.CompletionOptions The server provides completion support.
---@field public hoverProvider? (boolean | gindent.kit.LSP.HoverOptions) The server provides hover support.
---@field public signatureHelpProvider? gindent.kit.LSP.SignatureHelpOptions The server provides signature help support.
---@field public declarationProvider? (boolean | gindent.kit.LSP.DeclarationOptions | gindent.kit.LSP.DeclarationRegistrationOptions) The server provides Goto Declaration support.
---@field public definitionProvider? (boolean | gindent.kit.LSP.DefinitionOptions) The server provides goto definition support.
---@field public typeDefinitionProvider? (boolean | gindent.kit.LSP.TypeDefinitionOptions | gindent.kit.LSP.TypeDefinitionRegistrationOptions) The server provides Goto Type Definition support.
---@field public implementationProvider? (boolean | gindent.kit.LSP.ImplementationOptions | gindent.kit.LSP.ImplementationRegistrationOptions) The server provides Goto Implementation support.
---@field public referencesProvider? (boolean | gindent.kit.LSP.ReferenceOptions) The server provides find references support.
---@field public documentHighlightProvider? (boolean | gindent.kit.LSP.DocumentHighlightOptions) The server provides document highlight support.
---@field public documentSymbolProvider? (boolean | gindent.kit.LSP.DocumentSymbolOptions) The server provides document symbol support.
---@field public codeActionProvider? (boolean | gindent.kit.LSP.CodeActionOptions) The server provides code actions. CodeActionOptions may only be<br>specified if the client states that it supports<br>`codeActionLiteralSupport` in its initial `initialize` request.
---@field public codeLensProvider? gindent.kit.LSP.CodeLensOptions The server provides code lens.
---@field public documentLinkProvider? gindent.kit.LSP.DocumentLinkOptions The server provides document link support.
---@field public colorProvider? (boolean | gindent.kit.LSP.DocumentColorOptions | gindent.kit.LSP.DocumentColorRegistrationOptions) The server provides color provider support.
---@field public workspaceSymbolProvider? (boolean | gindent.kit.LSP.WorkspaceSymbolOptions) The server provides workspace symbol support.
---@field public documentFormattingProvider? (boolean | gindent.kit.LSP.DocumentFormattingOptions) The server provides document formatting.
---@field public documentRangeFormattingProvider? (boolean | gindent.kit.LSP.DocumentRangeFormattingOptions) The server provides document range formatting.
---@field public documentOnTypeFormattingProvider? gindent.kit.LSP.DocumentOnTypeFormattingOptions The server provides document formatting on typing.
---@field public renameProvider? (boolean | gindent.kit.LSP.RenameOptions) The server provides rename support. RenameOptions may only be<br>specified if the client states that it supports<br>`prepareSupport` in its initial `initialize` request.
---@field public foldingRangeProvider? (boolean | gindent.kit.LSP.FoldingRangeOptions | gindent.kit.LSP.FoldingRangeRegistrationOptions) The server provides folding provider support.
---@field public selectionRangeProvider? (boolean | gindent.kit.LSP.SelectionRangeOptions | gindent.kit.LSP.SelectionRangeRegistrationOptions) The server provides selection range support.
---@field public executeCommandProvider? gindent.kit.LSP.ExecuteCommandOptions The server provides execute command support.
---@field public callHierarchyProvider? (boolean | gindent.kit.LSP.CallHierarchyOptions | gindent.kit.LSP.CallHierarchyRegistrationOptions) The server provides call hierarchy support.<br><br>@since 3.16.0
---@field public linkedEditingRangeProvider? (boolean | gindent.kit.LSP.LinkedEditingRangeOptions | gindent.kit.LSP.LinkedEditingRangeRegistrationOptions) The server provides linked editing range support.<br><br>@since 3.16.0
---@field public semanticTokensProvider? (gindent.kit.LSP.SemanticTokensOptions | gindent.kit.LSP.SemanticTokensRegistrationOptions) The server provides semantic tokens support.<br><br>@since 3.16.0
---@field public monikerProvider? (boolean | gindent.kit.LSP.MonikerOptions | gindent.kit.LSP.MonikerRegistrationOptions) The server provides moniker support.<br><br>@since 3.16.0
---@field public typeHierarchyProvider? (boolean | gindent.kit.LSP.TypeHierarchyOptions | gindent.kit.LSP.TypeHierarchyRegistrationOptions) The server provides type hierarchy support.<br><br>@since 3.17.0
---@field public inlineValueProvider? (boolean | gindent.kit.LSP.InlineValueOptions | gindent.kit.LSP.InlineValueRegistrationOptions) The server provides inline values.<br><br>@since 3.17.0
---@field public inlayHintProvider? (boolean | gindent.kit.LSP.InlayHintOptions | gindent.kit.LSP.InlayHintRegistrationOptions) The server provides inlay hints.<br><br>@since 3.17.0
---@field public diagnosticProvider? (gindent.kit.LSP.DiagnosticOptions | gindent.kit.LSP.DiagnosticRegistrationOptions) The server has support for pull model diagnostics.<br><br>@since 3.17.0
---@field public inlineCompletionProvider? (boolean | gindent.kit.LSP.InlineCompletionOptions) Inline completion options used during static registration.<br><br>@since 3.18.0<br>@proposed
---@field public workspace? gindent.kit.LSP.WorkspaceOptions Workspace specific server capabilities.
---@field public experimental? gindent.kit.LSP.LSPAny Experimental server capabilities.

---@class gindent.kit.LSP.ServerInfo
...
---@field public type gindent.kit.LSP.FileChangeType The change type.

---@class gindent.kit.LSP.FileSystemWatcher
---@field public globPattern gindent.kit.LSP.GlobPattern The glob pattern to watch. See {@link GlobPattern glob pattern} for more detail.<br><br>@since 3.17.0 support for relative patterns.
---@field public kind? gindent.kit.LSP.WatchKind The kind of events of interest. If omitted it defaults<br>to WatchKind.Create | WatchKind.Change | WatchKind.Delete<br>which is 7.

---@class gindent.kit.LSP.Diagnostic
---@field public range gindent.kit.LSP.Range The range at which the message applies
---@field public severity? gindent.kit.LSP.DiagnosticSeverity The diagnostic's severity. To avoid interpretation mismatches when a<br>server is used with different clients it is highly recommended that servers<br>always provide a severity value.
---@field public code? (integer | string) The diagnostic's code, which usually appear in the user interface.
---@field public codeDescription? gindent.kit.LSP.CodeDescription An optional property to describe the error code.<br>Requires the code field (above) to be present/not null.<br><br>@since 3.16.0
...
---@field public tags? gindent.kit.LSP.DiagnosticTag[] Additional metadata about the diagnostic.<br><br>@since 3.15.0
---@field public relatedInformation? gindent.kit.LSP.DiagnosticRelatedInformation[] An array of related diagnostic information, e.g. when symbol-names within<br>a scope collide all definitions can be marked via this property.
---@field public data? gindent.kit.LSP.LSPAny A data entry field that is preserved between a `textDocument/publishDiagnostics`<br>notification and `textDocument/codeAction` request.<br><br>@since 3.16.0

---@class gindent.kit.LSP.CompletionContext
---@field public triggerKind gindent.kit.LSP.CompletionTriggerKind How the completion was triggered.
...
---@class gindent.kit.LSP.CompletionItemDefaults
---@field public commitCharacters? string[] A default commit character set.<br><br>@since 3.17.0
---@field public editRange? (gindent.kit.LSP.Range | gindent.kit.LSP.EditRangeWithInsertReplace) A default edit range.<br><br>@since 3.17.0
---@field public insertTextFormat? gindent.kit.LSP.InsertTextFormat A default insert text format.<br><br>@since 3.17.0
...
---@field public completionItem? gindent.kit.LSP.ServerCompletionItemOptions The server supports the following `CompletionItem` specific<br>capabilities.<br><br>@since 3.17.0
...
---@field public activeSignatureHelp? gindent.kit.LSP.SignatureHelp The currently active `SignatureHelp`.<br><br>The `activeSignatureHelp` has its `SignatureHelp.activeSignature` field updated based on<br>the user navigating through available signatures.
...
---@field public documentation? (string | gindent.kit.LSP.MarkupContent) The human-readable doc-comment of this signature. Will be shown<br>in the UI but can be omitted.
...
---@class gindent.kit.LSP.CodeActionDisabled
...
---@field public documentation? gindent.kit.LSP.CodeActionKindDocumentation[] Static documentation for a class of code actions.<br><br>Documentation from the provider should be shown in the code actions menu if either:<br><br>- Code actions of `kind` are requested by the editor. In this case, the editor will show the documentation that<br>  most closely matches the requested code action kind. For example, if a provider has documentation for<br>  both `Refactor` and `RefactorExtract`, when the user requests code actions for `RefactorExtract`,<br>  the editor will use the documentation for `RefactorExtract` instead of the documentation for `Refactor`.<br><br>- Any code actions of `kind` are returned by the provider.<br><br>At most one documentation entry should be shown per provider.<br><br>@since 3.18.0<br>@proposed
...
---@class gindent.kit.LSP.LocationUriOnly
---@field public uri string
...
---@class gindent.kit.LSP.PrepareRenamePlaceholder
---@field public range gindent.kit.LSP.Range
---@field public placeholder string

---@class gindent.kit.LSP.PrepareRenameDefaultBehavior
...
---@class gindent.kit.LSP.WorkspaceEditMetadata
...
---@class gindent.kit.LSP.SemanticTokensFullDelta
...
---@field public annotationId gindent.kit.LSP.ChangeAnnotationIdentifier The actual identifier of the change annotation

---@class gindent.kit.LSP.SnippetTextEdit
---@field public range gindent.kit.LSP.Range The range of the text document to be manipulated.
---@field public snippet gindent.kit.LSP.StringValue The snippet to be inserted.
---@field public annotationId? gindent.kit.LSP.ChangeAnnotationIdentifier The actual identifier of the snippet edit.

---@class gindent.kit.LSP.ResourceOperation
---@field public kind string The resource operation kind.
---@field public annotationId? gindent.kit.LSP.ChangeAnnotationIdentifier An optional annotation identifier describing the operation.<br><br>@since 3.16.0
...
---@field public matches? gindent.kit.LSP.FileOperationPatternKind Whether to match files or folders with this pattern.<br><br>Matches both if undefined.
---@field public options? gindent.kit.LSP.FileOperationPatternOptions Additional options used during matching.
...
---@field public kind gindent.kit.LSP.NotebookCellKind The cell's kind
---@field public document string The URI of the cell's text document<br>content.
---@field public metadata? gindent.kit.LSP.LSPObject Additional metadata stored with the cell.<br><br>Note: should always be an object literal (e.g. LSPObject)
---@field public executionSummary? gindent.kit.LSP.ExecutionSummary Additional execution summary information<br>if supported by the client.

---@class gindent.kit.LSP.NotebookDocumentFilterWithNotebook
---@field public notebook (string | gindent.kit.LSP.NotebookDocumentFilter) The notebook to be synced If a string<br>value is provided it matches against the<br>notebook type. '*' matches every notebook.
---@field public cells? gindent.kit.LSP.NotebookCellLanguage[] The cells of the matching notebook to be synced.

---@class gindent.kit.LSP.NotebookDocumentFilterWithCells
---@field public notebook? (string | gindent.kit.LSP.NotebookDocumentFilter) The notebook to be synced If a string<br>value is provided it matches against the<br>notebook type. '*' matches every notebook.
---@field public cells gindent.kit.LSP.NotebookCellLanguage[] The cells of the matching notebook to be synced.

---@class gindent.kit.LSP.NotebookDocumentCellChanges
---@field public structure? gindent.kit.LSP.NotebookDocumentCellChangeStructure Changes to the cell structure to add or<br>remove cells.
---@field public data? gindent.kit.LSP.NotebookCell[] Changes to notebook cells properties like its<br>kind, execution summary or metadata.
---@field public textContent? gindent.kit.LSP.NotebookDocumentCellContentChanges[] Changes to the text content of notebook cells.

---@class gindent.kit.LSP.SelectedCompletionInfo
---@field public range gindent.kit.LSP.Range The range that will be replaced if this completion item is accepted.
---@field public text string The text the range will be replaced with if this completion is accepted.

---@class gindent.kit.LSP.ClientInfo
...
---@field public workspace? gindent.kit.LSP.WorkspaceClientCapabilities Workspace specific client capabilities.
---@field public textDocument? gindent.kit.LSP.TextDocumentClientCapabilities Text document specific client capabilities.
---@field public notebookDocument? gindent.kit.LSP.NotebookDocumentClientCapabilities Capabilities specific to the notebook document support.<br><br>@since 3.17.0
---@field public window? gindent.kit.LSP.WindowClientCapabilities Window specific client capabilities.
---@field public general? gindent.kit.LSP.GeneralClientCapabilities General client capabilities.<br><br>@since 3.16.0
---@field public experimental? gindent.kit.LSP.LSPAny Experimental client capabilities.
...
---@field public change? gindent.kit.LSP.TextDocumentSyncKind Change notifications are sent to the server. See TextDocumentSyncKind.None, TextDocumentSyncKind.Full<br>and TextDocumentSyncKind.Incremental. If omitted it defaults to TextDocumentSyncKind.None.
...
---@field public save? (boolean | gindent.kit.LSP.SaveOptions) If present save notifications are sent to the server. If omitted the notification should not be<br>sent.

---@class gindent.kit.LSP.WorkspaceOptions
---@field public workspaceFolders? gindent.kit.LSP.WorkspaceFoldersServerCapabilities The server supports workspace folder.<br><br>@since 3.6.0
---@field public fileOperations? gindent.kit.LSP.FileOperationOptions The server is interested in notifications/requests for operations on files.<br><br>@since 3.16.0

---@class gindent.kit.LSP.TextDocumentContentChangePartial
---@field public range gindent.kit.LSP.Range The range of the document that changed.
...
---@class gindent.kit.LSP.TextDocumentContentChangeWholeDocument
...
---@field public location gindent.kit.LSP.Location The location of this related diagnostic information.
---@field public message string The message of this related diagnostic information.

---@class gindent.kit.LSP.EditRangeWithInsertReplace
---@field public insert gindent.kit.LSP.Range
---@field public replace gindent.kit.LSP.Range

---@class gindent.kit.LSP.ServerCompletionItemOptions
...
---@class gindent.kit.LSP.MarkedStringWithLanguage
...
---@field public documentation? (string | gindent.kit.LSP.MarkupContent) The human-readable doc-comment of this parameter. Will be shown<br>in the UI but can be omitted.

---@class gindent.kit.LSP.CodeActionKindDocumentation
---@field public kind gindent.kit.LSP.CodeActionKind The kind of the code action being documented.<br><br>If the kind is generic, such as `CodeActionKind.Refactor`, the documentation will be shown whenever any<br>refactorings are returned. If the kind if more specific, such as `CodeActionKind.RefactorExtract`, the<br>documentation will only be shown when extract refactoring code actions are returned.
---@field public command gindent.kit.LSP.Command Command that is ued to display the documentation to the user.<br><br>The title of this documentation code action is taken from {@linkcode Command.title}

---@class gindent.kit.LSP.NotebookCellTextDocumentFilter
---@field public notebook (string | gindent.kit.LSP.NotebookDocumentFilter) A filter that matches against the notebook<br>containing the notebook cell. If a string<br>value is provided it matches against the<br>notebook type. '*' matches every notebook.
...
---@class gindent.kit.LSP.NotebookCellLanguage
---@field public language string

---@class gindent.kit.LSP.NotebookDocumentCellChangeStructure
---@field public array gindent.kit.LSP.NotebookCellArrayChange The change to the cell array.
---@field public didOpen? gindent.kit.LSP.TextDocumentItem[] Additional opened cell text documents.
---@field public didClose? gindent.kit.LSP.TextDocumentIdentifier[] Additional closed cell text documents.

---@class gindent.kit.LSP.NotebookDocumentCellContentChanges
---@field public document gindent.kit.LSP.VersionedTextDocumentIdentifier
---@field public changes gindent.kit.LSP.TextDocumentContentChangeEvent[]
...
---@field public workspaceEdit? gindent.kit.LSP.WorkspaceEditClientCapabilities Capabilities specific to `WorkspaceEdit`s.
---@field public didChangeConfiguration? gindent.kit.LSP.DidChangeConfigurationClientCapabilities Capabilities specific to the `workspace/didChangeConfiguration` notification.
---@field public didChangeWatchedFiles? gindent.kit.LSP.DidChangeWatchedFilesClientCapabilities Capabilities specific to the `workspace/didChangeWatchedFiles` notification.
---@field public symbol? gindent.kit.LSP.WorkspaceSymbolClientCapabilities Capabilities specific to the `workspace/symbol` request.
---@field public executeCommand? gindent.kit.LSP.ExecuteCommandClientCapabilities Capabilities specific to the `workspace/executeCommand` request.
...
---@field public semanticTokens? gindent.kit.LSP.SemanticTokensWorkspaceClientCapabilities Capabilities specific to the semantic token requests scoped to the<br>workspace.<br><br>@since 3.16.0.
---@field public codeLens? gindent.kit.LSP.CodeLensWorkspaceClientCapabilities Capabilities specific to the code lens requests scoped to the<br>workspace.<br><br>@since 3.16.0.
---@field public fileOperations? gindent.kit.LSP.FileOperationClientCapabilities The client has support for file notifications/requests for user operations on files.<br><br>Since 3.16.0
---@field public inlineValue? gindent.kit.LSP.InlineValueWorkspaceClientCapabilities Capabilities specific to the inline values requests scoped to the<br>workspace.<br><br>@since 3.17.0.
---@field public inlayHint? gindent.kit.LSP.InlayHintWorkspaceClientCapabilities Capabilities specific to the inlay hint requests scoped to the<br>workspace.<br><br>@since 3.17.0.
---@field public diagnostics? gindent.kit.LSP.DiagnosticWorkspaceClientCapabilities Capabilities specific to the diagnostic requests scoped to the<br>workspace.<br><br>@since 3.17.0.
---@field public foldingRange? gindent.kit.LSP.FoldingRangeWorkspaceClientCapabilities Capabilities specific to the folding range requests scoped to the workspace.<br><br>@since 3.18.0<br>@proposed

---@class gindent.kit.LSP.TextDocumentClientCapabilities
---@field public synchronization? gindent.kit.LSP.TextDocumentSyncClientCapabilities Defines which synchronization capabilities the client supports.
---@field public completion? gindent.kit.LSP.CompletionClientCapabilities Capabilities specific to the `textDocument/completion` request.
---@field public hover? gindent.kit.LSP.HoverClientCapabilities Capabilities specific to the `textDocument/hover` request.
---@field public signatureHelp? gindent.kit.LSP.SignatureHelpClientCapabilities Capabilities specific to the `textDocument/signatureHelp` request.
---@field public declaration? gindent.kit.LSP.DeclarationClientCapabilities Capabilities specific to the `textDocument/declaration` request.<br><br>@since 3.14.0
---@field public definition? gindent.kit.LSP.DefinitionClientCapabilities Capabilities specific to the `textDocument/definition` request.
---@field public typeDefinition? gindent.kit.LSP.TypeDefinitionClientCapabilities Capabilities specific to the `textDocument/typeDefinition` request.<br><br>@since 3.6.0
---@field public implementation? gindent.kit.LSP.ImplementationClientCapabilities Capabilities specific to the `textDocument/implementation` request.<br><br>@since 3.6.0
---@field public references? gindent.kit.LSP.ReferenceClientCapabilities Capabilities specific to the `textDocument/references` request.
---@field public documentHighlight? gindent.kit.LSP.DocumentHighlightClientCapabilities Capabilities specific to the `textDocument/documentHighlight` request.
---@field public documentSymbol? gindent.kit.LSP.DocumentSymbolClientCapabilities Capabilities specific to the `textDocument/documentSymbol` request.
---@field public codeAction? gindent.kit.LSP.CodeActionClientCapabilities Capabilities specific to the `textDocument/codeAction` request.
---@field public codeLens? gindent.kit.LSP.CodeLensClientCapabilities Capabilities specific to the `textDocument/codeLens` request.
---@field public documentLink? gindent.kit.LSP.DocumentLinkClientCapabilities Capabilities specific to the `textDocument/documentLink` request.
---@field public colorProvider? gindent.kit.LSP.DocumentColorClientCapabilities Capabilities specific to the `textDocument/documentColor` and the<br>`textDocument/colorPresentation` request.<br><br>@since 3.6.0
---@field public formatting? gindent.kit.LSP.DocumentFormattingClientCapabilities Capabilities specific to the `textDocument/formatting` request.
---@field public rangeFormatting? gindent.kit.LSP.DocumentRangeFormattingClientCapabilities Capabilities specific to the `textDocument/rangeFormatting` request.
---@field public onTypeFormatting? gindent.kit.LSP.DocumentOnTypeFormattingClientCapabilities Capabilities specific to the `textDocument/onTypeFormatting` request.
---@field public rename? gindent.kit.LSP.RenameClientCapabilities Capabilities specific to the `textDocument/rename` request.
---@field public foldingRange? gindent.kit.LSP.FoldingRangeClientCapabilities Capabilities specific to the `textDocument/foldingRange` request.<br><br>@since 3.10.0
---@field public selectionRange? gindent.kit.LSP.SelectionRangeClientCapabilities Capabilities specific to the `textDocument/selectionRange` request.<br><br>@since 3.15.0
---@field public publishDiagnostics? gindent.kit.LSP.PublishDiagnosticsClientCapabilities Capabilities specific to the `textDocument/publishDiagnostics` notification.
---@field public callHierarchy? gindent.kit.LSP.CallHierarchyClientCapabilities Capabilities specific to the various call hierarchy requests.<br><br>@since 3.16.0
---@field public semanticTokens? gindent.kit.LSP.SemanticTokensClientCapabilities Capabilities specific to the various semantic token request.<br><br>@since 3.16.0
---@field public linkedEditingRange? gindent.kit.LSP.LinkedEditingRangeClientCapabilities Capabilities specific to the `textDocument/linkedEditingRange` request.<br><br>@since 3.16.0
---@field public moniker? gindent.kit.LSP.MonikerClientCapabilities Client capabilities specific to the `textDocument/moniker` request.<br><br>@since 3.16.0
---@field public typeHierarchy? gindent.kit.LSP.TypeHierarchyClientCapabilities Capabilities specific to the various type hierarchy requests.<br><br>@since 3.17.0
---@field public inlineValue? gindent.kit.LSP.InlineValueClientCapabilities Capabilities specific to the `textDocument/inlineValue` request.<br><br>@since 3.17.0
---@field public inlayHint? gindent.kit.LSP.InlayHintClientCapabilities Capabilities specific to the `textDocument/inlayHint` request.<br><br>@since 3.17.0
---@field public diagnostic? gindent.kit.LSP.DiagnosticClientCapabilities Capabilities specific to the diagnostic pull model.<br><br>@since 3.17.0
---@field public inlineCompletion? gindent.kit.LSP.InlineCompletionClientCapabilities Client capabilities specific to inline completions.<br><br>@since 3.18.0<br>@proposed

---@class gindent.kit.LSP.NotebookDocumentClientCapabilities
---@field public synchronization gindent.kit.LSP.NotebookDocumentSyncClientCapabilities Capabilities specific to notebook document synchronization<br><br>@since 3.17.0
...
---@field public showMessage? gindent.kit.LSP.ShowMessageRequestClientCapabilities Capabilities specific to the showMessage request.<br><br>@since 3.16.0
---@field public showDocument? gindent.kit.LSP.ShowDocumentClientCapabilities Capabilities specific to the showDocument request.<br><br>@since 3.16.0

---@class gindent.kit.LSP.GeneralClientCapabilities
---@field public staleRequestSupport? gindent.kit.LSP.StaleRequestSupportOptions Client capability that signals how the client<br>handles stale requests (e.g. a request<br>for which the client will not process the response<br>anymore since the information is outdated).<br><br>@since 3.17.0
---@field public regularExpressions? gindent.kit.LSP.RegularExpressionsClientCapabilities Client capabilities specific to regular expressions.<br><br>@since 3.16.0
---@field public markdown? gindent.kit.LSP.MarkdownClientCapabilities Client capabilities specific to the client's markdown parser.<br><br>@since 3.16.0
---@field public positionEncodings? gindent.kit.LSP.PositionEncodingKind[] The position encodings supported by the client. Client and server<br>have to agree on the same position encoding to ensure that offsets<br>(e.g. character position in a line) are interpreted the same on both<br>sides.<br><br>To keep the protocol backwards compatible the following applies: if<br>the value 'utf-16' is missing from the array of position encodings<br>servers can assume that the client supports UTF-16. UTF-16 is<br>therefore a mandatory encoding.<br><br>If omitted it defaults to ['utf-16'].<br><br>Implementation considerations: since the conversion from one encoding<br>into another requires the content of the file / line the conversion<br>is best done where the file is read which is usually on the server<br>side.<br><br>@since 3.17.0
...
---@field public didCreate? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving didCreateFiles notifications.
---@field public willCreate? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving willCreateFiles requests.
---@field public didRename? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving didRenameFiles notifications.
---@field public willRename? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving willRenameFiles requests.
---@field public didDelete? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving didDeleteFiles file notifications.
---@field public willDelete? gindent.kit.LSP.FileOperationRegistrationOptions The server is interested in receiving willDeleteFiles file requests.

---@class gindent.kit.LSP.RelativePattern
---@field public baseUri (gindent.kit.LSP.WorkspaceFolder | string) A workspace folder or a base URI to which this pattern will be matched<br>against relatively.
---@field public pattern gindent.kit.LSP.Pattern The actual glob pattern;

---@class gindent.kit.LSP.TextDocumentFilterLanguage
...
---@field public pattern? gindent.kit.LSP.GlobPattern A glob pattern, like **​/*.{ts,js}. See TextDocumentFilter for examples.<br><br>@since 3.18.0 - support for relative patterns.

---@class gindent.kit.LSP.TextDocumentFilterScheme
---@field public language? string A language id, like `typescript`.
---@field public scheme string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field public pattern? gindent.kit.LSP.GlobPattern A glob pattern, like **​/*.{ts,js}. See TextDocumentFilter for examples.<br><br>@since 3.18.0 - support for relative patterns.

---@class gindent.kit.LSP.TextDocumentFilterPattern
---@field public language? string A language id, like `typescript`.
---@field public scheme? string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field public pattern gindent.kit.LSP.GlobPattern A glob pattern, like **​/*.{ts,js}. See TextDocumentFilter for examples.<br><br>@since 3.18.0 - support for relative patterns.

---@class gindent.kit.LSP.NotebookDocumentFilterNotebookType
---@field public notebookType string The type of the enclosing notebook.
---@field public scheme? string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field public pattern? gindent.kit.LSP.GlobPattern A glob pattern.

---@class gindent.kit.LSP.NotebookDocumentFilterScheme
---@field public notebookType? string The type of the enclosing notebook.
---@field public scheme string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field public pattern? gindent.kit.LSP.GlobPattern A glob pattern.

---@class gindent.kit.LSP.NotebookDocumentFilterPattern
---@field public notebookType? string The type of the enclosing notebook.
---@field public scheme? string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field public pattern gindent.kit.LSP.GlobPattern A glob pattern.
...
---@field public cells? gindent.kit.LSP.NotebookCell[] The new cells, if any
...
---@field public resourceOperations? gindent.kit.LSP.ResourceOperationKind[] The resource operations the client supports. Clients should at least<br>support 'create', 'rename' and 'delete' files and folders.<br><br>@since 3.13.0
---@field public failureHandling? gindent.kit.LSP.FailureHandlingKind The failure handling strategy of a client if applying the workspace edit<br>fails.<br><br>@since 3.13.0
...
---@field public changeAnnotationSupport? gindent.kit.LSP.ChangeAnnotationsSupportOptions Whether the client in general supports change annotations on text edits,<br>create file, rename file and delete file changes.<br><br>@since 3.16.0
...
---@field public symbolKind? gindent.kit.LSP.ClientSymbolKindOptions Specific capabilities for the `SymbolKind` in the `workspace/symbol` request.
---@field public tagSupport? gindent.kit.LSP.ClientSymbolTagOptions The client supports tags on `SymbolInformation`.<br>Clients supporting tags have to handle unknown tags gracefully.<br><br>@since 3.16.0
---@field public resolveSupport? gindent.kit.LSP.ClientSymbolResolveOptions The client support partial workspace symbols. The client will send the<br>request `workspaceSymbol/resolve` to the server to resolve additional<br>properties.<br><br>@since 3.17.0
...
---@class gindent.kit.LSP.FoldingRangeWorkspaceClientCapabilities
...
---@field public completionItem? gindent.kit.LSP.ClientCompletionItemOptions The client supports the following `CompletionItem` specific<br>capabilities.
---@field public completionItemKind? gindent.kit.LSP.ClientCompletionItemOptionsKind
---@field public insertTextMode? gindent.kit.LSP.InsertTextMode Defines how the client handles whitespace and indentation<br>when accepting a completion item that uses multi line<br>text in either `insertText` or `textEdit`.<br><br>@since 3.17.0
...
---@field public completionList? gindent.kit.LSP.CompletionListCapabilities The client supports the following `CompletionList` specific<br>capabilities.<br><br>@since 3.17.0
...
---@field public contentFormat? gindent.kit.LSP.MarkupKind[] Client supports the following content formats for the content<br>property. The order describes the preferred format of the client.
...
---@field public signatureInformation? gindent.kit.LSP.ClientSignatureInformationOptions The client supports the following `SignatureInformation`<br>specific properties.
...
---@field public symbolKind? gindent.kit.LSP.ClientSymbolKindOptions Specific capabilities for the `SymbolKind` in the<br>`textDocument/documentSymbol` request.
---@field public hierarchicalDocumentSymbolSupport? boolean The client supports hierarchical document symbols.
---@field public tagSupport? gindent.kit.LSP.ClientSymbolTagOptions The client supports tags on `SymbolInformation`. Tags are supported on<br>`DocumentSymbol` if `hierarchicalDocumentSymbolSupport` is set to true.<br>Clients supporting tags have to handle unknown tags gracefully.<br><br>@since 3.16.0
...
---@field public codeActionLiteralSupport? gindent.kit.LSP.ClientCodeActionLiteralOptions The client support code action literals of type `CodeAction` as a valid<br>response of the `textDocument/codeAction` request. If the property is not<br>set the request can only return `Command` literals.<br><br>@since 3.8.0
...
---@field public resolveSupport? gindent.kit.LSP.ClientCodeActionResolveOptions Whether the client supports resolving additional code action<br>properties via a separate `codeAction/resolve` request.<br><br>@since 3.16.0
...
---@field public tagSupport? gindent.kit.LSP.CodeActionTagOptions Client supports the tag property on a code action. Clients<br>supporting tags have to handle unknown tags gracefully.<br><br>@since 3.18.0 - proposed

---@class gindent.kit.LSP.CodeActionTagOptions
---@field public valueSet gindent.kit.LSP.CodeActionTag[] The tags supported by the client.
...
---@field public resolveSupport? gindent.kit.LSP.ClientCodeLensResolveOptions Whether the client supports resolving additional code lens<br>properties via a separate `codeLens/resolve` request.<br><br>@since 3.18.0
...
---@field public prepareSupportDefaultBehavior? gindent.kit.LSP.PrepareSupportDefaultBehavior Client supports the default behavior result.<br><br>The value indicates the default behavior used by the<br>client.<br><br>@since 3.16.0
...
---@field public foldingRangeKind? gindent.kit.LSP.ClientFoldingRangeKindOptions Specific options for the folding range kind.<br><br>@since 3.17.0
---@field public foldingRange? gindent.kit.LSP.ClientFoldingRangeOptions Specific options for the folding range.<br><br>@since 3.17.0
...
---@class gindent.kit.LSP.PublishDiagnosticsClientCapabilities : gindent.kit.LSP.DiagnosticsCapabilities
...
---@field public formats gindent.kit.LSP.TokenFormat[] The token formats the clients supports.
...
---@field public resolveSupport? gindent.kit.LSP.ClientInlayHintResolveOptions Indicates which properties a client can resolve lazily on an inlay<br>hint.

---@class gindent.kit.LSP.DiagnosticClientCapabilities : gindent.kit.LSP.DiagnosticsCapabilities
---@field public dynamicRegistration? boolean Whether implementation supports dynamic registration. If this is set to `true`<br>the client supports the new `(TextDocumentRegistrationOptions & StaticRegistrationOptions)`<br>return value for the corresponding server capability as well.
---@field public relatedDocumentSupport? boolean Whether the clients supports related documents for document diagnostic pulls.

---@class gindent.kit.LSP.InlineCompletionClientCapabilities
...
---@field public messageActionItem? gindent.kit.LSP.ClientShowMessageActionItemOptions Capabilities specific to the `MessageActionItem` type.
...
---@class gindent.kit.LSP.StaleRequestSupportOptions
...
---@field public engine gindent.kit.LSP.RegularExpressionEngineKind The engine's name.
...
---@class gindent.kit.LSP.ChangeAnnotationsSupportOptions
...
---@class gindent.kit.LSP.ClientSymbolKindOptions
---@field public valueSet? gindent.kit.LSP.SymbolKind[] The symbol kind values the client supports. When this<br>property exists the client also guarantees that it will<br>handle values outside its set gracefully and falls back<br>to a default value when unknown.<br><br>If this property is not present the client only supports<br>the symbol kinds from `File` to `Array` as defined in<br>the initial version of the protocol.

---@class gindent.kit.LSP.ClientSymbolTagOptions
---@field public valueSet gindent.kit.LSP.SymbolTag[] The tags supported by the client.

---@class gindent.kit.LSP.ClientSymbolResolveOptions
---@field public properties string[] The properties that a client can resolve lazily. Usually<br>`location.range`

---@class gindent.kit.LSP.ClientCompletionItemOptions
...
---@field public documentationFormat? gindent.kit.LSP.MarkupKind[] Client supports the following content formats for the documentation<br>property. The order describes the preferred format of the client.
...
---@field public resolveSupport? gindent.kit.LSP.ClientCompletionItemResolveOptions Indicates which properties a client can resolve lazily on a completion<br>item. Before version 3.16.0 only the predefined properties `documentation`<br>and `details` could be resolved lazily.<br><br>@since 3.16.0
---@field public insertTextModeSupport? gindent.kit.LSP.ClientCompletionItemInsertTextModeOptions The client supports the `insertTextMode` property on<br>a completion item to override the whitespace handling mode<br>as defined by the client (see `insertTextMode`).<br><br>@since 3.16.0
...
---@class gindent.kit.LSP.ClientCompletionItemOptionsKind
---@field public valueSet? gindent.kit.LSP.CompletionItemKind[] The completion item kind values the client supports. When this<br>property exists the client also guarantees that it will<br>handle values outside its set gracefully and falls back<br>to a default value when unknown.<br><br>If this property is not present the client only supports<br>the completion items kinds from `Text` to `Reference` as defined in<br>the initial version of the protocol.

---@class gindent.kit.LSP.CompletionListCapabilities
...
---@class gindent.kit.LSP.ClientSignatureInformationOptions
---@field public documentationFormat? gindent.kit.LSP.MarkupKind[] Client supports the following content formats for the documentation<br>property. The order describes the preferred format of the client.
---@field public parameterInformation? gindent.kit.LSP.ClientSignatureParameterInformationOptions Client capabilities specific to parameter information.
...
---@class gindent.kit.LSP.ClientCodeActionLiteralOptions
---@field public codeActionKind gindent.kit.LSP.ClientCodeActionKindOptions The code action kind is support with the following value<br>set.

---@class gindent.kit.LSP.ClientCodeActionResolveOptions
---@field public properties string[] The properties that a client can resolve lazily.

---@class gindent.kit.LSP.ClientCodeLensResolveOptions
---@field public properties string[] The properties that a client can resolve lazily.

---@class gindent.kit.LSP.ClientFoldingRangeKindOptions
---@field public valueSet? gindent.kit.LSP.FoldingRangeKind[] The folding range kind values the client supports. When this<br>property exists the client also guarantees that it will<br>handle values outside its set gracefully and falls back<br>to a default value when unknown.

---@class gindent.kit.LSP.ClientFoldingRangeOptions
...
---@class gindent.kit.LSP.DiagnosticsCapabilities
---@field public relatedInformation? boolean Whether the clients accepts diagnostics with related information.
---@field public tagSupport? gindent.kit.LSP.ClientDiagnosticsTagOptions Client supports the tag property to provide meta data about a diagnostic.<br>Clients supporting tags have to handle unknown tags gracefully.<br><br>@since 3.15.0
...
---@class gindent.kit.LSP.ClientSemanticTokensRequestOptions
...
---@field public full? (boolean | gindent.kit.LSP.ClientSemanticTokensRequestFullDelta) The client will send the `textDocument/semanticTokens/full` request if<br>the server provides a corresponding handler.

---@class gindent.kit.LSP.ClientInlayHintResolveOptions
---@field public properties string[] The properties that a client can resolve lazily.

---@class gindent.kit.LSP.ClientShowMessageActionItemOptions
...
---@class gindent.kit.LSP.CompletionItemTagOptions
---@field public valueSet gindent.kit.LSP.CompletionItemTag[] The tags supported by the client.

---@class gindent.kit.LSP.ClientCompletionItemResolveOptions
---@field public properties string[] The properties that a client can resolve lazily.

---@class gindent.kit.LSP.ClientCompletionItemInsertTextModeOptions
---@field public valueSet gindent.kit.LSP.InsertTextMode[]

---@class gindent.kit.LSP.ClientSignatureParameterInformationOptions
...
---@class gindent.kit.LSP.ClientCodeActionKindOptions
---@field public valueSet gindent.kit.LSP.CodeActionKind[] The code action kind values the client supports. When this<br>property exists the client also guarantees that it will<br>handle values outside its set gracefully and falls back<br>to a default value when unknown.

---@class gindent.kit.LSP.ClientDiagnosticsTagOptions
---@field public valueSet gindent.kit.LSP.DiagnosticTag[] The tags supported by the client.

---@class gindent.kit.LSP.ClientSemanticTokensRequestFullDelta
...
---@alias gindent.kit.LSP.WorkspaceFoldingRangeRefreshResponse nil
...
---@alias gindent.kit.LSP.TextDocumentInlineCompletionResponse (gindent.kit.LSP.InlineCompletionList | gindent.kit.LSP.InlineCompletionItem[] | nil)
...
---@alias gindent.kit.LSP.TextDocumentRangesFormattingResponse (gindent.kit.LSP.TextEdit[] | nil)
...
---@alias gindent.kit.LSP.PrepareRenameResult (gindent.kit.LSP.Range | gindent.kit.LSP.PrepareRenamePlaceholder | gindent.kit.LSP.PrepareRenameDefaultBehavior)
...
---@alias gindent.kit.LSP.TextDocumentContentChangeEvent (gindent.kit.LSP.TextDocumentContentChangePartial | gindent.kit.LSP.TextDocumentContentChangeWholeDocument)

---@alias gindent.kit.LSP.MarkedString (string | gindent.kit.LSP.MarkedStringWithLanguage)

---@alias gindent.kit.LSP.DocumentFilter (gindent.kit.LSP.TextDocumentFilter | gindent.kit.LSP.NotebookCellTextDocumentFilter)

---@alias gindent.kit.LSP.LSPObject table<string, gindent.kit.LSP.LSPAny>

---@alias gindent.kit.LSP.GlobPattern (gindent.kit.LSP.Pattern | gindent.kit.LSP.RelativePattern)

---@alias gindent.kit.LSP.TextDocumentFilter (gindent.kit.LSP.TextDocumentFilterLanguage | gindent.kit.LSP.TextDocumentFilterScheme | gindent.kit.LSP.TextDocumentFilterPattern)

---@alias gindent.kit.LSP.NotebookDocumentFilter (gindent.kit.LSP.NotebookDocumentFilterNotebookType | gindent.kit.LSP.NotebookDocumentFilterScheme | gindent.kit.LSP.NotebookDocumentFilterPattern)

---@alias gindent.kit.LSP.Pattern string

---@alias gindent.kit.LSP.RegularExpressionEngineKind string
